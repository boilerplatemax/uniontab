...
import { ChevronUp, ChevronDown, Loader2, UserPlus, Eye } from 'lucide-react';
...
export function MemberLoginDropdown({ slug, contactEmail, isDemo = false }: MemberLoginDropdownProps) {
  ...
  const handleDemoLogin = async () => {
    ...
    setError('');
    try {
      const response = await fetch(`/api/union/${slug}/demo-login`, {
        method: 'POST',
      });
  ...
  return (
    <div className="relative">
      ...
      >
        {isDemo ? 'Demo Login' : 'Member Login'}
        {isOpen ? (
      ...
      {isOpen && (
        ...
        >
          {isDemo ? (
            ...
            <div className="p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="font-semibold text-gray-900">Demo View</h3>
              </div>
              <p className="text-sm text-gray-600 mb-4">
                Explore the admin dashboard with sample data. All actions are disabled — this is a read-only tour of the platform.
              </p>
              <ul className="text-xs text-gray-500 mb-5 space-y-1">
                <li className="flex items-center gap-1.5">
                  <span className="text-green-500">✓</span> View all admin tools and reports
                </li>
                <li className="flex items-center gap-1.5">
                  <span className="text-green-500">✓</span> Browse member list, news, events &amp; files
                </li>
                <li className="flex items-center gap-1.5">
                  <span className="text-green-500">✓</span> See grievance and election management
                </li>
                <li className="flex items-center gap-1.5">
                  <span className="text-amber-500">✗</span> No post, edit, or delete actions
                </li>
              </ul>
              ...
              <Button
                ...
                disabled={loading}
                className="w-full bg-amber-500 hover:bg-amber-600 text-gray-900 font-semibold py-2 gap-2"
              >
                {loading ? (
                  <>
          ...
          {/* Join Union Section — hidden for demo unions */}
          {!isDemo && (
