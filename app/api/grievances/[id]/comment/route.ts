...
import { grievances, members, grievanceComments, grievanceParticipants } from '@/lib/db/schema';
...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    if (!isOwnerOrAdmin && !isGrievanceCreator && !isAssignedMember && !isParticipant) {
      return NextResponse.json(
        { error: 'You do not have permission to comment on this grievance' },
        { status: 403 }
      );
    }

    // Only admins/owners can create internal notes
    const shouldBeInternal = isInternal && isOwnerOrAdmin;

    // Create the comment
    const [newComment] = await db
      .insert(grievanceComments)
      .values({
        grievanceId,
        comment,
