import { Translations } from '../types';

export const fr: Translations = {
  landing: {
    nav: {
      features: 'Fonctionnalités',
      pricing: 'Tarifs',
      about: 'À propos',
      blog: 'Blogue',
      contact: 'Contact',
      memberLogin: 'Connexion membre',
      getStarted: 'Commencer',
    },
    hero: {
      title: {
        highlight: 'Outils modernes',
        rest: 'pour syndicats modernes',
      },
      subtitle: 'Une plateforme pour engager les membres, organiser des élections sécurisées et économiser des heures chaque semaine.',
      stats: {
        builtByUnion: 'Créé par des syndicalistes',
        uptime: 'Disponibilité 99,9%',
        freeForSmall: 'Gratuit pour les petites sections',
      },
      cta: {
        startFree: 'Commencer gratuitement',
        seeFeatures: 'Voir les fonctionnalités',
      },
      floatingCard: {
        title: 'Gratuit pour les petites sections',
        subtitle: 'Moins de 150 membres',
      },
    },
    howItWorks: {
      title: 'Démarrez en 3 étapes simples',
      subtitle: 'La plupart des syndicats sont opérationnels en moins de 30 minutes.',
      steps: {
        step1: {
          title: 'Créez votre profil syndical',
          description: 'Configurez le portail de votre syndicat en quelques minutes. Ajoutez votre image de marque, vos statuts et personnalisez l\'expérience de vos membres.',
        },
        step2: {
          title: 'Invitez vos membres',
          description: 'Vos membres peuvent s\'inscrire directement sur votre portail syndical, ou invitez-les par courriel.',
        },
        step3: {
          title: 'Commencez à communiquer',
          description: 'Envoyez votre première communication, planifiez une élection ou partagez des documents importants avec vos membres.',
        },
      },
    },
    features: {
      title: 'Tout ce dont vous avez besoin pour diriger',
      subtitle: 'Des outils puissants conçus spécifiquement pour les dirigeants syndicaux afin d\'engager les membres, organiser des élections et bâtir des communautés plus fortes.',
      elections: {
        title: 'Élections sécurisées',
        description: 'Organisez des élections démocratiques en toute confiance. Notre plateforme de vote sécurisée garantit la transparence et l\'intégrité de toutes les décisions syndicales.',
        points: ['Vote anonyme et vérifiable', 'Suivi des résultats en temps réel', 'Pistes d\'audit et conformité'],
      },
      communications: {
        title: 'Communications de masse',
        description: 'Envoyez des mises à jour par courriel et SMS à tous vos membres ou à des groupes spécifiques. Tenez tout le monde informé avec des bulletins professionnels et des alertes urgentes.',
        points: ['Messagerie courriel et SMS en masse', 'Modèles personnalisables', 'Suivi des livraisons et analytiques'],
      },
      memberPortal: {
        title: 'Portail des membres',
        description: 'Offrez à vos membres un espace sécurisé pour accéder aux ressources syndicales, consulter leurs avantages et rester connectés avec la direction.',
        points: ['Authentification sécurisée des membres', 'Bibliothèque de documents et ressources', 'Tableau de bord personnel'],
      },
      analytics: {
        title: 'Analytiques et aperçus',
        description: 'Prenez des décisions basées sur les données avec des analytiques complètes sur l\'engagement des membres, la participation aux événements et l\'efficacité des communications.',
        points: ['Métriques d\'engagement', 'Rapports et exports personnalisés', 'Analyse des tendances'],
      },
      storage: {
        title: 'Stockage sécurisé de documents',
        description: 'Stockez les contrats, statuts et documents importants dans un système sécurisé et organisé. Contrôlez qui peut accéder à quoi avec des permissions granulaires.',
        points: ['Stockage cloud chiffré', 'Contrôle de version et historique', 'Gestion des permissions'],
      },
      events: {
        title: 'Gestion des événements',
        description: 'Planifiez des réunions, rassemblements et événements avec suivi RSVP intégré et rappels automatisés pour toutes vos activités syndicales.',
        points: ['Calendrier et RSVP', 'Rappels automatisés', 'Suivi des présences'],
      },
    },
    whyUs: {
      title: 'Pourquoi les syndicats choisissent UnionTab',
      subtitle: 'Conçu spécifiquement pour les dirigeants syndicaux par des gens qui comprennent vos défis',
      builtByUnion: {
        title: 'Créé par des syndicalistes',
        description: 'Nous comprenons les défis uniques de la direction syndicale parce que nous les avons vécus. Chaque fonctionnalité est conçue avec vos besoins réels en tête.',
      },
      freeForSmall: {
        title: 'Gratuit pour les petites sections',
        description: 'Nous croyons que chaque syndicat mérite des outils modernes. Les sections locales de moins de 150 membres ont accès gratuitement aux fonctionnalités de base, pour toujours.',
      },
      saveHours: {
        title: 'Économisez des heures chaque semaine',
        description: 'Arrêtez de jongler entre les feuilles de calcul, les listes de courriels et les bulletins de vote papier. Consolidez vos opérations sur une seule plateforme et reprenez votre temps.',
      },
    },
    security: {
      title: 'Conçu pour la sécurité et la conformité',
      subtitle: 'Les données de vos membres et l\'intégrité des élections sont nos priorités',
      encryption: {
        title: 'Chiffrement de bout en bout',
        description: 'Toutes les données chiffrées en transit et au repos',
      },
      auditTrails: {
        title: 'Pistes d\'audit',
        description: 'Dossiers complets pour la conformité réglementaire',
      },
      anonymousVoting: {
        title: 'Vote anonyme',
        description: 'Résultats vérifiables, confidentialité protégée',
      },
      uptime: {
        title: 'Disponibilité 99,9%',
        description: 'Accès fiable quand vous en avez le plus besoin',
      },
    },
    trustedBy: 'Fait confiance par les sections syndicales',
    cta: {
      title: 'Prêt à moderniser votre syndicat?',
      subtitle: 'Commencez à bâtir une adhésion plus forte et plus connectée dès aujourd\'hui. La plupart des syndicats sont opérationnels en moins de 30 minutes.',
      startFree: 'Commencer gratuitement',
      viewPricing: 'Voir les tarifs',
      bottomText: 'Gratuit pour les sections de moins de 150 membres  ·  Aucune carte de crédit requise  ·  Configuration en moins de 30 minutes',
      demoText: 'Vous voulez voir en action? Nous vous encourageons à l\'essayer vous-même, mais nous serons heureux de vous guider.',
      bookDemo: 'Réserver une démo',
    },
    footer: {
      description: 'Outiller les syndicats avec des outils numériques modernes pour bâtir des communautés plus fortes et plus connectées.',
      product: 'Produit',
      company: 'Entreprise',
      legal: 'Légal',
      about: 'À propos',
      privacy: 'Politique de confidentialité',
      terms: 'Conditions d\'utilisation',
      copyright: '© 2026 UnionTab. Tous droits réservés.',
    },
  },
};
